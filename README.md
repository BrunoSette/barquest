...
Ontario Bar Prep Ltd. has no affiliation with the Law Society of Ontario and the questions on our practice exams were not designed to be a prediction of what will be asked on any upcoming bar exam. Although we feel that taking our practice exams will help you study for the bar exam, WE MAKE NO GUARANTEE THAT YOU WILL PASS THE ONTARIO BARRISTER OR SOLICITOR EXAM.
...
- [BarQuest](#barquest)
- [Next.js SaaS Starter](#nextjs-saas-starter)
...
We're able to fetch the user from our Postgres database in the root layout, but _not_ await the `Promise`. Instead, we forward the `Promise` to a React context provider, where we can "unwrap" it and await the streamed-in data. This means we can have the best of both worlds: easy code to fetch data from our database (e.g. `getUser()`) and a React hook we can use in Client Components (e.g. `useUser()`).
